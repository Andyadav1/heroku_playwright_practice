...
test("Add/Remove_Elements", async ({ page }) => {
  ...
  //await addElement.click();
  //while loop is also possible but not recommended as its infinite and may break or cause memory  issues
...
test(`Challenging_DOM`, async ({ page }) => {
  ...
  //button locators
  let blue_btn = page.locator('//a[@class="button"]');
  let red_btn = page.locator('//a[@class="button alert"]');
  let green_btn = page.locator('//a[@class="button success"]');
